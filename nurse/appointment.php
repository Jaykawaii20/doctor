...
if (isset($_SESSION["user"])) {
    if ($_SESSION["user"] == "" || $_SESSION['usertype'] != 'n') {
...
$userrow = $database->query("SELECT * FROM nurses WHERE email='$useremail'");
$userfetch = $userrow->fetch_assoc();

$userid = $userfetch["nurse_id"]; // Assuming the primary key is `nurse_id`
$username = $userfetch["fullname"]; // Assuming you want to fetch the full name
        ...
        <center>
            <div class="abc scroll">
                <table width="93%" class="sub-table scrolldown" border="0">
                    <thead>
                        <tr>
                            <th class="table-headin" style="padding: 15px;">
                                Patient name
                            </th>
                            <th class="table-headin" style="padding: 15px;">
                                Appointment number
                        ...
                        if ($result->num_rows == 0) {
                            ...
                            for ($x = 0; $x < $result->num_rows; $x++) {
                                ...
                                echo '<tr style="height: 50px;">
                                        <td style="font-weight:600; padding: 10px;"> &nbsp;' . substr($pname, 0, 25) . '</td>
                                        <td style="text-align:center; font-size:23px; font-weight:500; color: var(--btnnicetext); padding: 10px;">
                                            ' . $apponum . '
                                        </td>
                                        ...
                                        <td style="padding: 10px;">
                                            <div style="display:flex; justify-content: center;">
                                                <a href="?action=drop&id=' . $appoid . '&name=' . $pname . '&session=' . $title . '&apponum=' . $apponum . '" class="non-style-link">
                                                    <button class="btn-primary-soft btn button-icon btn-delete" style="padding: 10px 20px; margin-top: 10px;">Cancel</button>
                                                </a>
                                            </div>
                                        </td>
                                    </tr>';
