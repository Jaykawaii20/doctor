...
<body>
        ...
        $userrow = $database->query("SELECT * FROM midwives WHERE email='$useremail'"); // Changed the query to midwives
...
$sql = "SELECT ap.id AS appointment_id, ap.appointmentname, ap.date AS appointment_date, ap.datecreated, 
        p.name, p.middlename, p.lastname, p.address, p.age, p.Gender 
        FROM appointment_patient ap
        INNER JOIN patient p ON ap.pid = p.pid
        WHERE ap.status = 'approved' AND ap.appointmentname = 'midwives'";  // Add condition to fetch only approved appointments
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while($row = $result->fetch_assoc()) {
                            echo "<tr>
                                    ...
                                    <td>{$row['Gender']}</td>
                                    <td><a href='#' class='btn btn-primary btn-sm'>Session</a></td>
                                  </tr>";
                        }
                    } else {
